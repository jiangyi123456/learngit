...
encoder_a.add(LSTM(256, input_shape=(x_train.shape[1],x_train.shape[2]),return_sequences=True))
encoder_a.add(BatchNormalization())
encoder_a.add(LSTM(128, input_shape=(x_train.shape[1],x_train.shape[2]),return_sequences=True))
encoder_a.add(BatchNormalization())
encoder_a.add(LSTM(64, input_shape=(x_train.shape[1],x_train.shape[2])))
encoder_a.add(BatchNormalization())
encoder_b = Sequential()
encoder_b.add(LSTM(256, input_shape=(x_train.shape[1],x_train.shape[2]),return_sequences=True))
encoder_b.add(BatchNormalization())
encoder_b.add(LSTM(128, input_shape=(x_train.shape[1],x_train.shape[2]),return_sequences=True))
encoder_b.add(BatchNormalization())
encoder_b.add(LSTM(64, input_shape=(x_train.shape[1],x_train.shape[2])))
encoder_b.add(BatchNormalization())

decoder = Sequential()
decoder.add(Merge([encoder_a, encoder_b], mode='concat')) # output is 200
decoder.add(Dense(64, activation='relu',kernel_initializer='glorot_uniform'))
decoder.add(Dropout(rate=0.5))
decoder.add(Dense(19,activation='relu',kernel_initializer='random_uniform'))

##############################################

history = LossHistory() # 实例化类
if save_weights_flag:
    adam = keras.optimizers.adam(lr=1e-3) # 学习率0.001
    ...
    decoder.compile(loss='mae', optimizer='adam', metrics=['accuracy'])

value = decoder.predict([x_test,x_test1],batch_size=64)
value1=np.round(value)
value1=abs(int(value-y_test))
